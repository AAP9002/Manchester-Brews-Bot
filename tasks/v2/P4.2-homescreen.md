...
| Low on beans | POST `LOW_ON_BEANS_WEBHOOK` (env var); navigate to `success` with `message=MESSAGES["low_on_beans"]`, `return_to="home"` |
...
class HomeScreen:
    ...
    def _fire_low_beans(self):
        webhook = os.getenv("LOW_ON_BEANS_WEBHOOK")  # confirm exact env-var name in settings.toml
        self._send.post(webhook)  # ignore (ok, body) — success screen always shows
...
Verify the actual env-var key for the low-beans webhook by reading `settings.toml` / `settings.example.toml` — match what's there. If it doesn't exist, add `LOW_ON_BEANS_WEBHOOK` to `settings.example.toml` as part of this task.
...
- [ ] Low on beans → POSTs the webhook then navigates to `"success"` with the formatted two-line message; subtitle reads `"<LOW_BEANS_PERSON> has been peer-pressured effectively..."`, defaulting to `"Someone"` if the env var is unset.
